...
Item {
    ...
    implicitWidth: Config.notification.sizes.width + padding * 2
    implicitHeight: {
        const count = list.count;
        if (count === 0)
            return 0;

        let height = (count - 1) * Config.style.spacingSmaller;
    ...
    ClippingWrapperRectangle {
        ...
        StyledListView {
            ...
            delegate: Item {
                ...
                readonly property int nonAnimHeight: notif.nonAnimHeight
                property int idx
                ...
                implicitHeight: notif.implicitHeight + (idx === 0 ? 0 : Config.style.spacingSmaller)
                ...
                SequentialAnimation {
                    ...
                    Anim {
                        target: notif
                        property: "x"
                        to: (notif.x >= 0 ? Config.notification.sizes.width : -Config.notification.sizes.width) * 2
                ...
                ClippingRectangle {
                    anchors.top: parent.top
                    anchors.topMargin: wrapper.idx === 0 ? 0 : Config.style.spacingSmaller
            ...
            ExtraIndicator {
                ...
                extra: {
                    ...
                    let height = 0;
                    for (let i = 0; i < count; i++) {
                        height += (list.itemAtIndex(i)?.nonAnimHeight ?? 0) + Config.style.spacingSmaller;

                        if (height - Config.style.spacingSmaller >= scrollY)
                            return i;
                    }
